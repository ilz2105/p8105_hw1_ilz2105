...
Bewlow I created a data frame that has:
...
I found the mean for `mean_samp` and `mean_log`, but could not get the mean
for `mean_char` and `mean_fac` because they were not numeric values.
The mean for `mean_samp` is `r mean(pull(df_1, ran_samp))`, and the mean 
for `mean_log` is `r mean(pull(df_1, log_vec))`.
...
```
The product of `log_vec` after being converted to numeric and `ran_samp` is `r as.numeric(pull(df_1, log_vec))*(pull(df_1, ran_samp))`. 
We cannot mutiply a factor, so trying to convert `log_vec` to a factor and then multiplying
...
variables, `num_log` and `fac_vec`, respectively.
